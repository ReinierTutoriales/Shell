...
- [ ] Record a clean Release build baseline.
- [ ] Record baseline menu latency/memory/handle metrics.

## Phase 1 — System Environment

- [ ] Consolidate Windows theme/high-contrast/transparency state into a single snapshot.
- [ ] Add capability detection for Windows 11 visual behavior without repeated registry/API queries in paint hot paths.
- [ ] Preserve existing NSS overrides above system-derived defaults.
- [ ] Refresh the snapshot on the relevant system-setting/theme notifications.
- [ ] Add architecture checks/tests for system-default -> semantic-theme consumption.
...
2. Introduce the system-environment snapshot by extending the existing production abstraction rather than creating a parallel renderer.
3. Wire the snapshot into the actual theme construction path.
4. Consolidate semantic Windows 11 metrics/palette.
5. Move icon resolution to system-first policy.
6. Harden Explorer boundaries.
7. Profile and optimize measured hot paths.
